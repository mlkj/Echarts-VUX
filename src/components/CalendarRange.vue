<template>
    <div>
        <cell :title="title" is-link :arrow-direction="panelState ? 'up' : 'down'">
            <div slot="value">
                <span @click="togglePanel" style="color: green" v-text="range ? value[0] + ' ~ ' + value[1] : value"></span>
            </div>
        </cell>
        <transition name="toggle">
            <div class="date-panel" v-show="panelState">
                ...
                <div class="type-year" v-show="panelType === 'year'">
                    <ul class="year-list">
                        <li v-for="(item,index) in yearList" v-text="item" :key="index" :class="{selected: isSelected('year', item), invalid: validateYear(item)}" @click="selectYear(item)">
                        </li>
                    </ul>
                </div>
                <div class="type-month" v-show="panelType === 'month'">
                    <ul class="month-list">
                        <li v-for="(item, index) in monthList" :key="index" :class="{selected: isSelected('month', index), invalid: validateMonth(index)}" @click="selectMonth(index)">
                            {{item | month(language)}}
                        </li>
                    </ul>
                </div>
                <div class="type-date" v-show="panelType === 'date'">
                    <ul class="weeks">
                        <li v-for="item in weekList" :key="item.id">{{item | week(language)}}</li>
                    </ul>
                    <ul class="date-list">
